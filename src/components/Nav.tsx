  ...
  return (
    <div className={`flex items-center gap-4 ${className}`}>
      {socialLinks?.instagram && (
        <a href={socialLinks.instagram} target="_blank" rel="noreferrer" className="transition-colors hover:text-gold">
      ...
      {socialLinks?.facebook && (
        <a href={socialLinks.facebook} target="_blank" rel="noreferrer" className="transition-colors hover:text-gold">
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
        </a>
      )}
      {socialLinks?.youtube && (
        <a href={socialLinks.youtube} target="_blank" rel="noreferrer" className="transition-colors hover:text-gold">
        ...
        <div className="mx-auto relative flex max-w-7xl items-center justify-between px-6 py-4 md:py-5">
          ...
          <div className="flex flex-1 justify-end items-center gap-5 xl:gap-8">
            ...
            <div className="flex items-center">
              <SocialIcons socialLinks={fallbackLinks} className={`${headerDark ? "text-ink/80" : "text-bone/85"}`} />
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="mx-auto flex h-full max-w-7xl flex-col justify-start gap-10 overflow-y-auto px-8 pt-36 pb-32 md:flex-row md:items-center md:justify-center md:gap-24 md:pt-24 md:pb-16">
          ...
          <div className="mt-4 md:mt-0">
            ...
            <div className={`mt-8 transition-all duration-700 ${open ? "translate-y-0 opacity-100" : "translate-y-6 opacity-0"}`} style={{ transitionDelay: '500ms' }}>
              ...
              <SocialIcons socialLinks={fallbackLinks} className="text-bone/70" />
            </div>
          </div>
        </div>
      </div>
